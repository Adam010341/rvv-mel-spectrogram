...
void fft(float *__restrict real, float *__restrict imag, size_t n) {
    ...
    for(size_t len=2;len<=n;len<<=1){
        ...
        double angle_step=(-2*M_PI)/(double)len;
        ...
        for(size_t i=0;i<n;i+=len){
            ...
            for(size_t j=0;j<half_len;){
                ...
                vfloat32m8_t t_re=__riscv_vfmul_vv_f32m8(v_re,w_re,vl);//(a+jb)*(c+jd)=(ac-bd)+j(bc+ad)
                t_re=__riscv_vfnmsac_vv_f32m8(t_re,v_im,w_im,vl);

                vfloat32m8_t t_im=__riscv_vfmul_vv_f32m8(v_im,w_re,vl);//(a+jb)*(c+jd)=(ac-bd)+j(bc+ad)
                t_im=__riscv_vfmacc_vv_f32m8(t_im,v_re,w_im,vl);
...
void power_spectrum(const float *__restrict stft_data, size_t num_frames,
            ...
            size_t total=num_frames*N_FREQ_BINS;
